...
class ActorNetwork(object):
    ...
    def choose_action(self, s):
        s = s[np.newaxis, :]
        probes = self.sess.run(self.acts_prob, {self.s: s})  # get probabilities for all actions
        # Below Will Cause Error: act return as a TUPLE, not a int.
        # DONT ADD COMMA! type(1) == int, type(1,) == tuple
        # return np.random.choice(np.arange(probes.shape[1]), p=probes.ravel()),  # return a int
        return np.random.choice(np.arange(probes.shape[1]), p=probes.ravel())  # return a int
...
class CriticNetwork(object):
    def __init__(self, sess, n_features, lr=0.01):
        ...
        with tf.variable_scope('Critic'):
            ...
            # calc TD_Error = (r + gamma * V_next) - V_eval \
...
for i_episode in range(MAX_EPISODES):
    ...
    while True:
        ...
        actor.learn(s, act, td_error)  # true_gradient = grad[logPi(s,a) * td_error]
        s = s_
        t += 1
